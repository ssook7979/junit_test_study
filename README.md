...
# 목 객체 사용
- 느린 것에 의존하는 부분(외부 API 등)은 클래스와의 결합도를 느슨하게 하여(생성자 주입, setter, 팩토리 메소드 등) Mock으로 대체할 수 있도록 한다.
## 사용 시 고려사항
- Mock이 프로덕션 코드를 올바르게 묘사하고 있는가
- 프로덕션 코드는 생각하지 못한 다른 형식으로 반환하는가?
- 프로덕션 코든느 예외를 던지는가?
- null을 반환하는가?
- Mock이 제대로 대체하고 있는지(프로덕션 코드가 동작하고 있는건 아닌지) 확인해보기 위해서는 RuntimeException을 던져본다.
- 실제 클래스의 종단 간 사용성을 보여주는 적절한 상위 테스트가 있는지 확인해본다.
	- Mock으로 대체된 곳은 테스트 커버리지에서 간극(gap)을 통합테스트와 같은 적절한 상위테스트가 필요하다.
# 테스트 리팩토링
